...
public class GetPersonInfoCommandTest {
    @Test
    public void commandCheck() throws Exception {
        GetPersonInfoCommand command = new GetPersonInfoCommand();
        Assert.assertEquals("getPersonInfo", command.getName());
        Assert.assertNull(command.getParams());
        Assert.assertFalse(command.isAsync());
    }

    @Test
    public void storeChack() throws Exception {
        GetPersonInfoCommand command = new GetPersonInfoCommand();
        ...
        GetPersonInfoCommand.CREATOR.createFromParcel(parcel);
        Assert.assertEquals(position, parcel.dataPosition());
        GetPersonInfoCommand[] commands = GetPersonInfoCommand.CREATOR.newArray(5);
